 ...
 * Mob   : +919727206702
 **/
class CompletedVisitFragment : Fragment(R.layout.fragment_visit_status_list) {
    ...
    private fun loadCompletedVisits() {
        val sessionManager = SessionManager(requireContext())
        viewMode.completedVisits(0, 20, sessionManager.providerID).observe(viewLifecycleOwner) {
            viewMode.handleResponse(it) { visits ->
                Timber.d { "Completed Visit ${Gson().toJson(visits)}" }
            }
        }
    }

    companion object {
        fun newInstance(): CompletedVisitFragment {
            return CompletedVisitFragment()
        }
    }
}