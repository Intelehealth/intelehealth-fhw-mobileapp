...
//        db.close();
    ...
    public static String getStartVisitNoteEncounterByVisitUUID(String visitUUID) {
        ...
        Cursor idCursor = db.rawQuery("SELECT uuid FROM tbl_encounter where visituuid = ? AND encounter_type_uuid = ? AND (sync = '1' OR sync = 'true' OR sync = 'TRUE') COLLATE NOCASE", new String[]{visitUUID, ENCOUNTER_VISIT_NOTE});
        if (idCursor.getCount() != 0) {
            while (idCursor.moveToNext()) {
    ...
    public boolean isCompletedExitedSurvey(String visitUUID) throws DAOException {
        ...
        try {
            ...
            Cursor idCursor = db.rawQuery("SELECT * FROM tbl_encounter where visituuid = ? and " + "encounter_type_uuid in ('629a9d0b-48eb-405e-953d-a5964c88dc30')",  // ENCOUNTER_PATIENT_EXIT_SURVEY
    ...
    public static String getEncounterIdForCompletedVisit(String visitUUID) throws DAOException {
        ...
        try {
            ...
            Cursor idCursor = db.rawQuery("SELECT uuid FROM tbl_encounter where visituuid = ? and " + "encounter_type_uuid in ('629a9d0b-48eb-405e-953d-a5964c88dc30','bd1fbfaa-f5fb-4ebd-b75c-564506fc309e')", new String[]{visitUUID}); // ENCOUNTER_PATIENT_EXIT_SURVEY
            if (idCursor.getCount() != 0) {
                while (idCursor.moveToNext()) {
                    encounterUuid = idCursor.getString(idCursor.getColumnIndexOrThrow("uuid"));
                }
    ...
    public static String getPrescriptionReceivedTime(String visitUUID) {
        ...
        Cursor idCursor = db.rawQuery("SELECT * FROM tbl_obs where encounteruuid = ? AND (sync = '1' OR sync = 'true' OR sync = 'TRUE') COLLATE NOCASE", new String[]{visitUUID});
        if (idCursor.getCount() != 0) {
            while (idCursor.moveToNext()) {
