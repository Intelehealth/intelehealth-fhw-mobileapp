...
//        }
    ...
    public boolean createPatients(PatientDTO patient, SQLiteDatabase db) throws DAOException {
        ...
        try {
            ...
            createdRecordsCount = db.insertWithOnConflict("tbl_patient", null, values,
    ...
    public boolean updatePatientToDB(PatientDTO patientDTO, String uuid) throws DAOException {
        ...
        try {
            ...
            Logger.logD("pulldata", "datadumper" + values);
            createdRecordsCount1 = db.update("tbl_patient", values, whereclause,
                    new String[]{uuid});
            db.setTransactionSuccessful();
            Logger.logD("created records", "created records count" + createdRecordsCount1);
        } catch (SQLException e) {
    ...
    public boolean updatePatientToDB1(Patient patientDTO, String uuid,
                                      List<PatientAttributesDTO> patientAttributesDTOS) throws DAOException {
        boolean isCreated = true;
        long createdRecordsCount1 = 0;
        SQLiteDatabase db
                = IntelehealthApplication.inteleHealthDatabaseHelper.getWritableDatabase();
    ...
    public boolean patientAttributes(
            List<PatientAttributesDTO> patientAttributesDTOS) throws DAOException {
        boolean isInserted = true;
        SQLiteDatabase db = IntelehealthApplication.inteleHealthDatabaseHelper.getWriteDb();
        db.beginTransaction();
        try {
            ...
            for (int i = 0; i < patientAttributesDTOS.size(); i++) {
                ...
                values.put("person_attribute_type_uuid",
                        patientAttributesDTOS.get(i).getPersonAttributeTypeUuid());
                ...
                db.insertWithOnConflict("tbl_patient_attribute", null, values,
                        SQLiteDatabase.CONFLICT_REPLACE);
            }
            db.setTransactionSuccessful();
        } catch (SQLException e) {
    ...
    public List<Attribute> getPatientAttributes(String patientuuid) throws DAOException {
        ...
        //db.beginTransaction();
        try {
            String query = "SELECT * from tbl_patient_attribute WHERE patientuuid= '" +
                           patientuuid + "'";
            ...
            if (cursor.moveToFirst()) {
                while (!cursor.isAfterLast()) {
                    ...
                    String attributeType = cursor.getString(
                            cursor.getColumnIndex("person_attribute_type_uuid"));
    ...
    public String getHouseHoldValue(String patientuuid) throws DAOException {
        ...
        try {
            Cursor idCursor = db.rawQuery(
                    "SELECT value FROM tbl_patient_attribute where patientuuid = ? AND " +
                    "person_attribute_type_uuid=? AND voided='0' COLLATE NOCASE",
    ...
    public List<String> getPatientUUIDs(String houseHoldValue) throws DAOException {
        ...
        try {
            Cursor cursor = db.rawQuery(
                    "SELECT patientuuid FROM tbl_patient_attribute where value = ? AND sync='0' " +
                    "COLLATE NOCASE",
    ...
    public List<FamilyMemberRes> getPatientName(String patientuuid) throws DAOException {
        ...
        try {
            Cursor cursor = db.rawQuery(
                    "SELECT openmrs_id,first_name,middle_name,last_name FROM tbl_patient where " +
                    "uuid = ? COLLATE NOCASE",
                    new String[]{patientuuid});
            if (cursor.getCount() != 0) {
                while (cursor.moveToNext()) {
                    familyMemberRes.setOpenMRSID(
                            cursor.getString(cursor.getColumnIndexOrThrow("openmrs_id")));
                    familyMemberRes.setName(
...
//                  middle_name = cursor.getString(cursor.getColumnIndexOrThrow("middle_name"));
    ...
    public String getAttributesName(String attributeuuid) throws DAOException {
        ...
        String name = "";
        try {
            String query = "SELECT name from tbl_patient_attribute_master WHERE uuid= '" +
                           attributeuuid + "'";
            Cursor cursor = db.rawQuery(query, null, null);
            if (cursor.moveToFirst()) {
                while (!cursor.isAfterLast()) {
    ...
    public boolean insertPatientAttributes(List<PatientAttributesDTO> patientAttributesDTOS,
                                           SQLiteDatabase db) throws DAOException {
    ...
    public boolean patinetAttributeMaster(
            List<PatientAttributeTypeMasterDTO> patientAttributeTypeMasterDTOS) throws DAOException {
        boolean isInserted = true;
        SQLiteDatabase db = IntelehealthApplication.inteleHealthDatabaseHelper.getWriteDb();
        db.beginTransaction();
        try {
            ...
            for (int i = 0; i < patientAttributeTypeMasterDTOS.size(); i++) {
                ...
                values.put("modified_date", AppConstants.dateAndTimeUtils.currentDateTime());
                values.put("sync", "TRUE");
                db.insertWithOnConflict("tbl_patient_attribute_master", null, values,
                        SQLiteDatabase.CONFLICT_REPLACE);
            }
            db.setTransactionSuccessful();
        } catch (SQLException e) {
    ...
    public String getUuidForAttribute(String attr) {
        ...
        SQLiteDatabase db
                = IntelehealthApplication.inteleHealthDatabaseHelper.getWritableDatabase();
        Cursor cursor = db.rawQuery(
                "SELECT uuid FROM tbl_patient_attribute_master where name = ? COLLATE NOCASE",
                new String[]{attr});
        if (cursor.getCount() != 0) {
            while (cursor.moveToNext()) {
    ...
    public boolean updateOpemmrsId(String openmrsId, String synced,
    ...
    public List<PatientDTO> unsyncedPatients() throws DAOException {
        ...
        try {
            Cursor idCursor = db.rawQuery(
                    "SELECT * FROM tbl_patient where (sync = ? OR sync=?) COLLATE NOCASE",
                    new String[]{"0", "false"});
            ...
            if (idCursor.getCount() != 0) {
                while (idCursor.moveToNext()) {
                    ...
                    patientDTO.setGuardianType(
                            idCursor.getString(idCursor.getColumnIndexOrThrow("guardian_type")));
                    patientDTO.setGuardianName(
                            idCursor.getString(idCursor.getColumnIndexOrThrow("guardian_name")));
                    patientDTO.setContactType(
                            idCursor.getString(idCursor.getColumnIndexOrThrow("contact_type")));
                    patientDTO.setEmContactName(
                            idCursor.getString(idCursor.getColumnIndexOrThrow("em_contact_name")));
                    patientDTO.setEmContactNumber(
                            idCursor.getString(idCursor.getColumnIndexOrThrow("em_contact_num")));
    ...
    public boolean updatePatientPhoto(String patientuuid,
                                      String profilePhotoPath) throws DAOException {
        boolean isUpdated = true;
    ...
    public String getOpenmrsId(String patientuuid) throws DAOException {
        ...
        try {
            Cursor cursor = db.rawQuery(
                    "SELECT openmrs_id FROM tbl_patient where uuid = ? COLLATE NOCASE",
                    new String[]{patientuuid});
            if (cursor.getCount() != 0) {
                while (cursor.moveToNext()) {
    ...
    public static List<PatientDTO> getAllPatientsFromDB(int limit, int offset) {
        ...
        final Cursor searchCursor = db.rawQuery(
                "SELECT * FROM " + table + " ORDER BY modified_date DESC limit ? offset ?",
                new String[]{String.valueOf(limit), String.valueOf(offset)});
        try {
            if (searchCursor.moveToFirst()) {
                do {
                    PatientDTO model = new PatientDTO();
                    model.setOpenmrsId(searchCursor.getString(
                            searchCursor.getColumnIndexOrThrow("openmrs_id")));
                    model.setFirstname(searchCursor.getString(
                            searchCursor.getColumnIndexOrThrow("first_name")));
                    model.setLastname(searchCursor.getString(
                            searchCursor.getColumnIndexOrThrow("last_name")));
                    model.setDateofbirth(searchCursor.getString(
                            searchCursor.getColumnIndexOrThrow("date_of_birth")));
                    model.setGender(
                            searchCursor.getString(searchCursor.getColumnIndexOrThrow("gender")));
                    model.setOpenmrsId(searchCursor.getString(
                            searchCursor.getColumnIndexOrThrow("openmrs_id")));
                    model.setMiddlename(searchCursor.getString(
                            searchCursor.getColumnIndexOrThrow("middle_name")));
                    model.setUuid(
                            searchCursor.getString(searchCursor.getColumnIndexOrThrow("uuid")));
                    model.setDateofbirth(searchCursor.getString(
                            searchCursor.getColumnIndexOrThrow("date_of_birth")));
                    model.setPhonenumber(StringUtils.mobileNumberEmpty
                            (phoneNumber(searchCursor.getString(
                                    searchCursor.getColumnIndexOrThrow("uuid")))));
                    model.setPatientPhoto(searchCursor.getString(
                            searchCursor.getColumnIndexOrThrow("patient_photo")));
                    model.setGuardianType(searchCursor.getString(
                            searchCursor.getColumnIndexOrThrow("guardian_type")));
                    model.setGuardianName(searchCursor.getString(
                            searchCursor.getColumnIndexOrThrow("guardian_name")));
                    model.setContactType(searchCursor.getString(
                            searchCursor.getColumnIndexOrThrow("contact_type")));
                    model.setEmContactName(searchCursor.getString(
                            searchCursor.getColumnIndexOrThrow("em_contact_name")));
                    model.setEmContactNumber(searchCursor.getString(
                            searchCursor.getColumnIndexOrThrow("em_contact_num")));
                    modelList.add(model);
    ...
    public static List<PatientDTO> getQueryPatients(String query) {
        ...
        final Cursor search_mobile_cursor = db.rawQuery(
                "SELECT DISTINCT patientuuid FROM tbl_patient_attribute WHERE value = ?",
                new String[]{search});
        /* DISTINCT will get remove the duplicate values. The duplicate value will come when you
        have created
         * a patient with mobile no. 12345 and patient is pushed than later you edit the mobile
         no to
         * 12344 or something. In this case, the local db maintains two separate rows both with
         value: 12344 */
        //if no data is present against that corresponding cursor than cursor count returns = 0 .
        // .. i.e cursor_count = 0 ...
        try {
        ...
        if (patientUUID_List.size() != 0) {
            for (int i = 0; i < patientUUID_List.size(); i++) {
                ...
                //  if(searchCursor.getCount() != -1) { //all values are present as per the
                //  search text entered...
                try {
                    if (searchCursor.moveToFirst()) {
                        do {
                            ...
                            model.setDateofbirth(searchCursor.getString(
                                    searchCursor.getColumnIndexOrThrow("date_of_birth")));
                            model.setGender(searchCursor.getString(
                                    searchCursor.getColumnIndexOrThrow("gender")));
                            model.setOpenmrsId(searchCursor.getString(
                                    searchCursor.getColumnIndexOrThrow("openmrs_id")));
                            model.setMiddlename(searchCursor.getString(
                                    searchCursor.getColumnIndexOrThrow("middle_name")));
                            model.setUuid(searchCursor.getString(
                                    searchCursor.getColumnIndexOrThrow("uuid")));
                            model.setDateofbirth(searchCursor.getString(
                                    searchCursor.getColumnIndexOrThrow("date_of_birth")));
                            model.setPhonenumber(StringUtils.mobileNumberEmpty
            ...
            final Cursor searchCursor = db.rawQuery(
                    "SELECT * FROM " + table + " WHERE first_name LIKE " + "'%" + search + "%' " +
                    "OR middle_name LIKE '%" + search + "%' OR last_name LIKE '%" + search +
                    "%' OR " +
                    "(first_name || middle_name) LIKE '%" + search +
                    "%' OR (middle_name || last_name) " +
                    "LIKE '%" + search + "%' OR (first_name || last_name) LIKE '%" + search + "%'" +
                    "OR first_name || ' ' || middle_name LIKE" + "'%" + search +
            ...
            //  if(searchCursor.getCount() != -1) { //all values are present as per the search
            //  text entered...
            try {
                if (searchCursor.moveToFirst()) {
                    do {
                        ...
                        model.setDateofbirth(searchCursor.getString(
                                searchCursor.getColumnIndexOrThrow("date_of_birth")));
                        model.setPhonenumber(StringUtils.mobileNumberEmpty(phoneNumber(
                                searchCursor.getString(
                                        searchCursor.getColumnIndexOrThrow("uuid")))));
                        model.setPatientPhoto(searchCursor.getString(
                                searchCursor.getColumnIndexOrThrow("patient_photo")));
                        model.setGuardianType(searchCursor.getString(
    ...
    public static Observable<List<PatientDTO>> getQueryPatientsObs(String query) {
        return Observable.create(emitter -> {
            ...
            final Cursor search_mobile_cursor = db.rawQuery(
                    "SELECT DISTINCT patientuuid FROM tbl_patient_attribute WHERE value = ?",
                    new String[]{search});
            /* DISTINCT will get remove the duplicate values. The duplicate value will come when
            you have created
             * a patient with mobile no. 12345 and patient is pushed than later you edit the
             mobile no to
             * 12344 or something. In this case, the local db maintains two separate rows both
             with value: 12344 */
            //if no data is present against that corresponding cursor than cursor count returns =
            // 0 ... i.e cursor_count = 0 ...
            try {
                if (search_mobile_cursor.moveToFirst()) {
                    do {
                        patientUUID_List.add(search_mobile_cursor.getString
            ...
            if (patientUUID_List.size() != 0) {
                ...
                final Cursor searchCursor = db.rawQuery(
                        ...
                        "OR middle_name || ' ' || last_name LIKE" + "'%" + search + "%'" +
                        "OR first_name || ' ' || last_name LIKE" + "'%" + search + "%'" +
                        " OR openmrs_id LIKE '%" + search + "%' " + "ORDER BY first_name ASC",
                        null);
    ...
    public static String phoneNumber(String patientuuid) throws DAOException {
        ...
        Cursor idCursor = db.rawQuery(
                "SELECT value FROM tbl_patient_attribute where patientuuid = ? AND " +
                "person_attribute_type_uuid='14d4f066-15f5-102d-96e4-000c29c2a5d7'",
                new String[]{patientuuid});
        try {
            if (idCursor.getCount() != 0) {
                while (idCursor.moveToNext()) {
    ...
    public static VisitDTO isVisitPresentForPatient_fetchVisitValues(String patientUUID) {
        ...
        SQLiteDatabase db
                = IntelehealthApplication.inteleHealthDatabaseHelper.getWritableDatabase();
        Cursor idCursor = db.rawQuery("SELECT * FROM tbl_visit WHERE patientuuid = ?",
                new String[]{patientUUID});
        try {
            if (idCursor.moveToFirst()) {
                do {
                    visitDTO.setUuid(idCursor.getString(idCursor.getColumnIndexOrThrow("uuid")));
                    visitDTO.setStartdate(
                            idCursor.getString(idCursor.getColumnIndexOrThrow("startdate")));
                    visitDTO.setEnddate(
                            idCursor.getString(idCursor.getColumnIndexOrThrow("enddate")));
                    String isSynced = idCursor.getString(idCursor.getColumnIndexOrThrow("sync"));
                    boolean sync = false;
                    if (isSynced != null) {
                        if (isSynced.equalsIgnoreCase("0") ||
                            isSynced.toLowerCase().equalsIgnoreCase("false"))
                            sync = false;
                        else if (isSynced.equalsIgnoreCase("1") ||
                                 isSynced.toLowerCase().equalsIgnoreCase("true"))
                            sync = true;
                    }
                    //CustomLog.d("TAG", "isVisitPresentForPatient_fetchVisitValues: " + sync);
                    visitDTO.setSyncd(sync);
                    //CustomLog.d("TAG", "isVisitPresentForPatient_fetchVisitValues: visit: " +
                    // visitDTO);
                }
                while (idCursor.moveToNext());
            }
    ...
    public static String[] getPatientDobAgeGender(String patientUuid) {
        ...
        final Cursor cursor = db.rawQuery("select * from tbl_patient where uuid = ? and " +
                                          "(sync = 1 OR sync = 'true' OR sync = 'TRUE') and " +
                                          "voided = 0",
                new String[]{patientUuid});
    ...
    public static PatientDTO getPatientDetailsByUuid(String patientUuid) {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                patientDTO.setCityvillage(
                        cursor.getString(cursor.getColumnIndexOrThrow("city_village")));
                patientDTO.setStateprovince(
                        cursor.getString(cursor.getColumnIndexOrThrow("state_province")));
                ...
                patientDTO.setGuardianType(
                        cursor.getString(cursor.getColumnIndexOrThrow("guardian_type")));
                patientDTO.setGuardianName(
                        cursor.getString(cursor.getColumnIndexOrThrow("guardian_name")));
                patientDTO.setContactType(
                        cursor.getString(cursor.getColumnIndexOrThrow("contact_type")));
                patientDTO.setEmContactName(
                        cursor.getString(cursor.getColumnIndexOrThrow("em_contact_name")));
                patientDTO.setEmContactNumber(
                        cursor.getString(cursor.getColumnIndexOrThrow("em_contact_num")));
...
//                }
    ...
    public PatientDTO retrievePatientDetails(Cursor cursor) {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                patientDTO.setEconomic(
                        cursor.getString(cursor.getColumnIndexOrThrow("economicStatus")));
                patientDTO.setEducation(
                        cursor.getString(cursor.getColumnIndexOrThrow("educationLevel")));
                patientDTO.setProviderUUID(
                ...
                patientDTO.setNationalID(
                        cursor.getString(cursor.getColumnIndexOrThrow("nationalId")));
                patientDTO.setProfileTimestamp(
                        cursor.getString(cursor.getColumnIndexOrThrow("profileImageTimestamp")));
                ...
                patientDTO.setTmhCaseNumber(
                        cursor.getString(cursor.getColumnIndexOrThrow("tmhCaseNumber")));
                patientDTO.setRequestId(
                        cursor.getString(cursor.getColumnIndexOrThrow("requestId")));
                patientDTO.setDiscipline(
                        cursor.getString(cursor.getColumnIndexOrThrow("discipline")));
                patientDTO.setRelativePhoneNumber(
                        cursor.getString(cursor.getColumnIndexOrThrow("relativePhoneNumber")));
                patientDTO.setDepartment(
                ...
                patientDTO.setRegistrationAddressOfHf(
                        cursor.getString(cursor.getColumnIndexOrThrow("registrationAddressOfHf")));
                patientDTO.setInn(cursor.getString(cursor.getColumnIndexOrThrow("inn")));
                patientDTO.setCodeOfHealthFacility(
                        cursor.getString(cursor.getColumnIndexOrThrow("codeOfHealthFacility")));
                patientDTO.setHealthFacilityName(
                        cursor.getString(cursor.getColumnIndexOrThrow("healthFacilityName")));
                patientDTO.setCodeOfDepartment(
                        cursor.getString(cursor.getColumnIndexOrThrow("codeOfDepartment")));
            } while (cursor.moveToNext());
        }
        cursor.close();
        return patientDTO;
    ...
    public static int getAllFollowupPatientCount() {
        ...
        String query =
                "SELECT a.uuid as visituuid, a.sync, a.patientuuid, substr(a.startdate, 1, 10) as" +
                " startdate, "
                + "date(substr(o.value, 1, 10)) as followup_date, o.value as follow_up_info,"
                + "b.patient_photo, a.enddate, b.uuid, b.first_name, "
                +
                "b.middle_name, b.last_name, b.date_of_birth, b.openmrs_id, b.gender, c.value AS " +
                "speciality, SUBSTR(o.value,1,10) AS value_text, MAX(o.obsservermodifieddate) AS " +
                "obsservermodifieddate "
                +
                "FROM tbl_visit a, tbl_patient b, tbl_encounter d, tbl_obs o, tbl_visit_attribute" +
                " c WHERE "
        ...
        final Cursor cursor = db.rawQuery(query, new String[]{
                UuidDictionary.FOLLOW_UP_VISIT
        });  //"e8caffd6-5d22-41c4-8d6a-bc31a44d0c86"
        if (cursor.moveToFirst()) {
            do {
                try {
                    String value_text = cursor.getString(
                            cursor.getColumnIndexOrThrow("value_text"));
                    count++;
                } catch (Exception e) {
                    e.printStackTrace();
                    CustomLog.e(TAG, e.getMessage());
