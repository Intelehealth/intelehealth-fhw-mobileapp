...
//        If no value is not found, then update fails so insert instead.
    ...
    public boolean updateFollowUpValue_Obs(ObsDTO obsDTO) {
        SQLiteDatabase db = AppConstants.inteleHealthDatabaseHelper.getWriteDb();
        db.beginTransaction();
        int updatedCount = 0;
        ContentValues values = new ContentValues();
...
//            values.put("conceptuuid", obsDTO.getConceptuuid());
            ...
            updatedCount = db.update("tbl_obs", values, selection, new String[]{
                    obsDTO.getEncounteruuid(), obsDTO.getConceptuuid()});
