...
public class ProviderProfileDao {
    ...
    private boolean createProvidersProfile(ProviderProfileDTO provider, SQLiteDatabase db) throws DAOException {
        ...
        try {
            ...
            createdRecordsCount = db.insertWithOnConflict("tbl_provider_profile", null, values, SQLiteDatabase.CONFLICT_REPLACE);
        } catch (SQLException e) {
            isCreated = false;
            throw new DAOException(e.getMessage(), e);
        } finally {
        ...
        try {
            ...
            if (cursor.getCount() != 0) {
                while (cursor.moveToNext()) {
                    ...
                    providerProfileDTO = new ProviderProfileDTO(provider_id, username, first_name, middle_name, last_name, gender, date_of_birth, age, phone_number, country_code, email, image_path);
    ...
    public boolean updateProfileDetails(ProviderProfileDTO provider, SQLiteDatabase db) {
        db.beginTransaction();
        int updatedCount = 0;
        ContentValues values = new ContentValues();
        String selection = "provider_id = ?";
        try {
            ...
            updatedCount = db.update("tbl_provider_profile", values, selection, new String[]{provider.getProvider_id()});
    ...
    public boolean updateProvidersProfile(List<ProviderProfileDTO> providerDTOS) throws DAOException {
