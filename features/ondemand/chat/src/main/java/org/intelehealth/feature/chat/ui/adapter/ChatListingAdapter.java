...
import static org.intelehealth.feature.chat.utils.ChatConstantKt.LEFT_ITEM_DOCT;
import static org.intelehealth.feature.chat.utils.ChatConstantKt.RIGHT_ITEM_HW;
...
// Medicine adapter <- Test <- Aid adapter <- Header adapter <- Recycler.Adapter
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (mItemList.get(position) instanceof ChatMessage) {
            ChatMessage message = (ChatMessage) mItemList.get(position);
            if (holder instanceof LeftViewHolder && message.getLayoutType() == LEFT_ITEM_DOCT) {
                LeftViewHolder leftViewHolder = (LeftViewHolder) holder;
                leftViewHolder.bind(message);
            } else if (holder instanceof RightViewHolder && message.getLayoutType() == RIGHT_ITEM_HW) {
