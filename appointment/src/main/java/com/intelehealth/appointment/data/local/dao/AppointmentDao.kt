...
/*
    @Query("SELECT count(*) from ida_localrecords.tbl_patient p, tbl_appointments a where p.uuid = a.patient_id AND a.status = 'booked' AND  (datetime(a.slot_js_date) >= datetime('now'))")
